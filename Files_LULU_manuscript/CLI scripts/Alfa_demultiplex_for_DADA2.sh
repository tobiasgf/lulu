#!/bin/bash
#### Description: Demultiplexing of fastq reads for processing with DADA2

#### Written by: Tobias Guldberg Frøslev - tobiasgf@bio.ku.dk  on 3-2017
...
while read TAG_NAME TAG_SEQ RTAG_SEQ; do
    ...
    # Trim tags, forward & reverse primers (search normal and antisens)
        ...
        ${CUTADAPT} -g "${PRIMER_F}" -O "${MIN_F}" - 2>> "${LOG}" > "${FINAL_FASTQ}"
    ...
    # Trim tags, forward & reverse primers (search normal and antisens)
        ...
        ${CUTADAPT} -g "${PRIMER_R}" -O "${MIN_R}" - 2>> "${LOG}" > "${FINAL_FASTQ}"
