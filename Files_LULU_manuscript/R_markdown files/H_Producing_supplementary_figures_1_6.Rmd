...
**Taxonomic dissimilarity between OTU methods and plant data.**  
```{r dpi=300, echo=FALSE}
tab_name <- file.path(main_path,"Table_taxonomic_dissimilatiry_long.txt")
gathered_dissimilarity_long <- 
 read.table(tab_name, sep="\t", header=TRUE, as.is=TRUE)
...
#Plot dissimilarity, full axes
ggplot(gathered_dissimilarity2,aes(x=raw,y=curated)) +
  geom_point(size=1,alpha=0.8,pch=21) + facet_grid(Method ~Level) +
  geom_abline(intercept = 0, linetype =2) + 
  xlab("Absolute taxonomic dissimilarity before curation") +
  ylab("Absolute taxonomic dissimilarity after curation") +
  scale_fill_brewer(palette = "Set1") +
  theme_bw()+
  theme(legend.position="none")+ theme(text = element_text(size=8)) + theme(axis.text.x = element_text(angle = 90, hjust = 1))
```

**Figure 6a.** Taxonomic dissimilarity between OTU method and plant data after curation is plotted on the y-axis. Taxonomic dissimilarity between OTU method and plant data before curation is plotted on the x-axis and. Tommunity dissimilarity is calculated on taxon/site tables reduced the genus level with observations corresponding to counts of species pr. genus pr. site, and dissimilarity as the absolute number of differences between OTU table and plant table pr. site. The dashed identity line is provided for evaluating whether the dissimilarity is lowered/improved (i.e. points to the right of the line) by curation. The “99/100%” clustering level denotes the pure DADA2 approach (100%) and SWARM with a d-value of 3 (99%). For most methods the curation with LULU lowers the taxonomic dissimilarity for most sites significantly, and only few sites are affected adversely, and then only very little. The effect on the CROP method is very low. (a) shows the full x-axis, (b) shows a x-axis truncated at 150 to facilitate comparison.   
...
ggplot(gathered_dissimilarity2,aes(x=raw,y=curated)) +
  geom_point(size=1,alpha=0.8,pch=21) + facet_grid(Method ~Level) +
  coord_cartesian(ylim = c(0, 150),xlim = c(0,300)) +
  geom_abline(intercept = 0, linetype =2) + 
  xlab("Absolute taxonomic dissimilarity before curation") +
  ylab("Absolute taxonomic dissimilarity after curation") +
  scale_fill_brewer(palette = "Set1") +
...
**Figure 6b (truncated x-axis).** Taxonomic dissimilarity between OTU method and plant data after curation is plotted on the y-axis. Taxonomic dissimilarity between OTU method and plant data before curation is plotted on the x-axis and. Taxonomic dissimilarity is calculated on taxon/site tables reduced the genus level with observations corresponding to counts of species pr. genus pr. site, and dissimilarity as the absolute number of differences between OTU table and plant table pr. site. The dashed identity line is provided for evaluating whether the dissimilarity is lowered/improved (i.e. points to the right of the line) by curation. The “99/100%” clustering level denotes the pure DADA2 approach (100%) and SWARM with a d-value of 3 (99%). For most methods the curation with LULU lowers the taxonomic dissimilarity for most sites significantly, and only few sites are affected adversely, and then only very little. The effect on the CROP method is very low. (a) shows the full x-axis, (b) shows a x-axis truncated at 150 to facilitate comparison.   
